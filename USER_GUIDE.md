...
5. 点击“拆分 JSON 并刷新列”。
6. 如果只需要拆分后的结构化数据，点击“下载拆分明细”。
7. 如果需要继续生成准召统计，在“最终结果列（模型分）”中选择用于计算阈值的分数字段，例如 `prob_1`。
8. 在“正确结果列”中选择人工结果或真实标签列。
9. 在“正例取值”中填写目标类别，例如：
...
11. 点击“生成统计与 Excel”。
12. 查看“分层分布”和“阈值准召”，或点击“下载处理结果”导出 Excel。
...
如果只点击“下载拆分明细”，导出的 Excel 会只包含拆分后的明细数据，便于自行做后续统计。

`ObjectID`、`对象ID` 等长 ID 列会以文本格式导出，避免 Excel 将长数字后几位改成 `0`。
